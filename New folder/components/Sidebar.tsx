  ...
  return (
    <div className="w-full h-full bg-slate-50 flex flex-col shadow-2xl border-r border-slate-200">
      <div className="p-8 pb-6 bg-white border-b border-slate-100">
        ...
        <p className="text-slate-500 text-sm">Hello, <span className="font-bold text-slate-700">{currentUser.name}</span>! block by block.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
        {selectedItem ? (
            ...
            <div className={`bg-white rounded-3xl overflow-hidden shadow-xl border border-slate-100 transition-all ${selectedItem.isFound ? 'opacity-80 grayscale-[0.5]' : ''}`}>
              ...
              <div className="p-8 pt-6 space-y-4">
                ...
                <div className="pt-6 border-t border-slate-100">
                  ...
                  <p className="text-[10px] text-slate-400 mt-4 text-center px-4 font-medium uppercase tracking-tighter">
                    {selectedItem.isFound 
                      ? `Disappearing in ${getRemainingTime(selectedItem.foundAt)}` 
                      : "Marking as found will let others know it's gone."}
                  </p>
                </div>
              </div>
            </div>
          ...
          <>
            ...
            {/* Back to Map button - top left, under Latest Spots header */}
            <button 
              onClick={onCloseDetail}
              className="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors group px-2 mt-2"
            >
              <div className="w-7 h-7 rounded-xl bg-slate-100 flex items-center justify-center mr-2 group-hover:bg-blue-50 transition-colors">
                    ...
                    <div className="w-16 h-16 rounded-2xl bg-slate-50 flex-shrink-0 overflow-hidden relative border border-slate-50 flex items-center justify-center">
                      ...
                      {item.isFound && (
                        <div className="absolute inset-0 bg-slate-900/40 flex items-center justify-center">
                          <span className="text-[8px] font-black text-white uppercase tracking-tighter">Collected</span>
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">
                          {item.isFound ? `Collected ${getRemainingTime(item.foundAt)}` : new Date(item.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
